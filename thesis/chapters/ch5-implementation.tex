...
\Cref{fig:round-comms} show the relationship between communication cost and population.
...
Hence, for the same experiment we also record the duration in seconds, which is shown on the vertical axis instead of communication cost and plot as before (\Cref{fig:consensus-duration}).
...
These result do not conform to analytical result in~\Cref{sec:cons-duration}.
...
The result in~\Cref{fig:tx-comms-random} reinforces our argument because it is a lot more stable because for every transaction it is almost always the case that an agreed fragment is needed to validate it.
...
The difference in magnitude between \Cref{fig:global-throughput-fixed} and \Cref{fig:global-throughput-random} is also interesting.
...
We demonstrate this issue from a different perspective in~\Cref{fig:backlog}.
...
Note that the horizontal ``lines'' where no new validated transactions are made are roughly 70 seconds, this matches with our result in~\Cref{fig:round-duration-fixed}.
