...
The primary innovations are (1) the consensus model which prevents double spending,
and (2) the incentive mechanism that encourages anyone (with adequate hardware) to participate in the network and keep it running.
...
For example, Steam\footnote{\url{https://store.steampowered.com/}} and even Amazon\footnote{Not directly, but via \url{https://purse.io/}.}.
...
It also solves the transaction malleability problem~\cite{bitcoinmalleability} and paves the way for off-chain transactions via Lightning Network~\cite{lightningnetwork}.
...
then that node can proof to any other node that it holds a set of transactions that compute (form a chain) to the checkpoint.
...
\begin{itemize}
    ...
    \item We implement Checkpoint Consensus with experiment with up to 1200 nodes,
