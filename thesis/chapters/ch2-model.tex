...
% TODO everyone has one public key?
Finally, we use the random oracle model, calls to the random oracle are denoted by $\textsf{H}(\cdot)$.
...
% TODO rework this -> agreement, validity, fairness, termination
\begin{enumerate}
...
\label{sec:bootstrap}
% TODO rework this -> bootstrap is simply setting the facilitators in a fair way, no consensus
...
% TODO setup phase begins with known facilitators from F_r
...
and should correspond to the fair lottery and consistent facilitators in \Cref{def:consensus}.
...
Continuing, upon receiving $\C_{r - 1}$ and at least $n - f$ valid signatures by $u$, $u$ performs two asks.
First, it computes the new facilitators using $\textsf{get\_facilitator}(C, n)$ (\Cref{alg:facilitator})
...
The facilitators must satisfy the validity, agreement and totality properties from \Cref{def:consensus}.
...
% TODO consensus phase ends when F_{r + 1} is selected
% TODO remove duplicate CP blocks
