...
Before describing our consensus protocol, we take a brief detour to explain \emph{atomic broadcast} primitive and contrast it with other well known primitives.
It is a fundamental building block of our consensus protocol.

\subsubsection*{Atomic broadcast}
Atomic broadcast is an especially useful primitive for blockchain systems.
It allows nodes to propose values (which can be transactions),
and the goal is to reach a common ordering of the values---roughly speaking,
the output is an ordered set of the propsal.
Concretely, atomic broadcast needs to satisfy the following properties (adapted from~\cite{miller2016honey}).
\begin{enumerate}
\item \emph{Agreement}:
    If any correct node outputs a value $v$, then every correct node outputs $v$.
\item \emph{Total order}:
    If one correct node has output the sequence of values $v_0, v_1, \dots, v_j$
    and another has output $v'_0, v'_1, \dots, v_j'$, then $v_i = v'_i$ for $i \le \min(j, j')$.
\item \emph{Liveness}:
    If $v$ is an input to all the correct nodes, then it is eventually output by all the correct nodes.
\end{enumerate}
The liveness property also implies censorship resilience,
i.e. an adversay cannot block some $v$ if all the correct nodes are committed to it.

The atomic broadcast properties can be seen as a stronger version of reliable broadcast.
In reliable broadcast (e.g. Bracha's reliable broadcast~\cite{bracha1984asynchronous}),
there is only one sender and the following properties must be satisfied.
\begin{enumerate}
    \item \emph{Agreement}:
        If any two correct nodes deliver $v$ and $v'$, then $v = v'$.
    \item \emph{Totality}:
        If any correct node delivers $v$, then all correct nodes deliver $v$.
    \item \emph{Validity}:
        If the sender is correct and inputs $v$, then all correct nodes deliver $v$.
\end{enumerate}
The main different here is that reliable broadcast does not guarantee order, it only guarantees delivery.
If there are more than one sender, or even a single sender with multiple messages,
the order of delivery may be different on different nodes.

Atomic broadcast also is not the same as Byzantine agreement (BA).
In particular, BA algorithms lets nodes propose a binary value,
then output a binary value with respect to the following properties.
\begin{enumerate}
    \item \emph{Agreement}:
        If any correct node outputs $v$, then every correct node outputs $v$.
    \item \emph{Termination}:
        If all correct nodes make a proposal, then every correct node outputs a value.
    \item \emph{Validity}:
        If any correct node outputs $v$, then at least one correct node proposed $v$.
\end{enumerate}
The primary difference here is that BA only considers only binary value,
and the goal is to reach agreement on the binary value according with respect to the validity property.
Whereas the agreement is on all the proposals from honest nodes in atomic broadcast.

The main drawback with atomic broadcast (and other similar protocols, i.e. reliable broadcast and Byzantine agreement)
is the high message complexity.
Typically, such protocols have the complexity of $O(n^2)$, where $n$ is the number of participants.
This may work with a small number of nodes, but it is infeasible for blockchain systems where thousands of nodes are involved.
...
TODO rework definitions, that is: fair lottery, consistent facilitator, agreement, totoal order and liveness
...
Second if the node is a facilitator, it waits for some duration $D$ and collect messages of type \texttt{cp\_msg}, where $D >> \Delta$.
After $D$ is elapsed, it begins the atomic broadcast algorithm.
...
Further, the bootstrap waits for duration $\Delta + 1$ in order to collect all the CP blocks,
here we wait for some duration $D$---a system parameter,
it must be much larger than $\Delta + 1$ to ensure that all honest \texttt{cp\_msg} are collected.
