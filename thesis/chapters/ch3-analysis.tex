...
\subsection{Bandwidth Per Transaction}
...
If we assume that every node performs transactions at a constant rate of $c$ per second.
Then $l = O(N) \cdot c$, where $O(N)$ is the message complexity of ACS (TODO explain this somewhere),
thus the bandwidth per transaction is $O(N)$.
