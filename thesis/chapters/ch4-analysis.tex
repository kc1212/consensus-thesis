...
\subsection{Data Volume Requirement for Transactions}
...
which we call the data volume per transaction.
...
the data volume per transaction is of $O(l)$,
...
we know that $D_{\text{acs}}$ is of $O(N)$, thus the data volume per transaction is $O(N)$.
