...
and the TrustChain validation protocol (\Cref{def:validation}) in~\Cref{sec:correctness}.
Further, we analyse the performance and especially the throughput to understand the scalability characteristics in~\Cref{sec:performance-analysis}.
Finally, we consider the case where the $n \ge 3t + 1$ assumption is violated and show the effect of it in our system in~\Cref{sec:highly-adversarial}.
...
For all rounds, $\Pi_c$ implements a TrustChain consensus protocol (\Cref{def:consensus}).
\end{theorem}
\begin{proof}
...
In this section, we build on top of \Cref{theorem:consensus} to show that our validation protocol described in \Cref{sec:vd-protocol} has the agreement and validity properties of a TrustChain validation protocol defined in \Cref{def:validation}.
