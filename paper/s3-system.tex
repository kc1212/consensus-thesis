...
one on each of the party's chain.
}
...
While this gives us a consensus on a representation of the global state, it does not imply consensus on transactions.
To this end,
we introduce a validation protocol that allows any nodes to challenge any other node to produce a set of TX blocks that computes to the CP block in consensus,
achieving implicit consensus on transactions.
...
it consists of one data structure---Extended TrustChain,
and three protocols---consensus protocol, transaction protocol and validation protocol.
The complete protocol can be seen as the concurrent composition of the sub-protocols.

\subsection{Extended TrustChain}
Each node $u$ has a public and private key pair---$pk_u$ and $sk_u$, and a chain $B_u$.
The chain consists of blocks $B_u = \{ b_{u, i} : i \in \{ 0, \dots, h - 1 \} \}$,
...
where 
$$a = \argmin_{k, k < i, \textsf{typeof}(b_{u,k}) = \gamma}(i - k)$$
$$b = \argmin_{k, k > i, \textsf{typeof}(b_{u,k}) = \gamma}(k - i),$$
...
Some TX blocks may not have any enclosure, then its enclosure is $\bot$.
...
because a failure in one round will lead to failures in subsequent rounds.
